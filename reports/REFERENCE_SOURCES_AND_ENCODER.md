...
**Model storage:** The RoBERTa/GraphMERT model is stored on disk. **Default path:** `load_model()` with no arguments loads from `~/.cache/llama-cpp/models/encoders/roberta-base` (directory with `checkpoint.json` or Hugging Face `config.json`). Override the root with `GRAPHMERT_ENCODER_ROOT`. For a custom path use `load_model(path)` and `save_model(model, path)`; the checkpoint file is JSON (config and metadata). Loading pretrained weights from the checkpoint file is post-MVP (see `specs/003-align-contextual-description/spec.md` Clarifications).
