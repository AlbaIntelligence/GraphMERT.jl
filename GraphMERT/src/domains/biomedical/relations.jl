...
It is self-contained and includes all relation types, classification, validation, and extraction logic.
"""
...
Validate that a relation is semantically valid (legacy enum version).
"""
...
Validate a biomedical relation (string-based version for domain interface).
"""
function validate_biomedical_relation(head::String, relation_type::String, tail::String, context::Dict{String, Any} = Dict{String, Any}())
    ...
    try
        ...
        if relation_type_enum != UNKNOWN_RELATION
            # For domain interface, use lenient validation - if relation type is valid,
            # accept it unless there's a clear reason to reject
            # Try strict validation first, but fall back to lenient if needed
            strict_result = validate_biomedical_relation(head, tail, relation_type_enum)
            if strict_result
                return true
            else
                # Lenient fallback: if relation type is valid and entities are non-empty, accept it
                # This allows for real entity names that don't contain pattern keywords
                return length(head) > 1 && length(tail) > 1
            end
        else
            return false
        end
...
Calculate confidence score for relation classification (legacy enum version).
"""
...
Calculate confidence for a biomedical relation (string-based version for domain interface).
"""
function calculate_biomedical_relation_confidence(head::String, relation_type::String, tail::String, context::Dict{String, Any} = Dict{String, Any}())
    ...
    try
        relation_type_enum = parse_relation_type(relation_type)
        if relation_type_enum != UNKNOWN_RELATION
            # Get context string from dict if available
            context_str = get(context, "context", "")
            return calculate_relation_confidence(head, tail, relation_type_enum, context_str)
        else
            return 0.3
        end
    catch
...
export BiomedicalRelationType
export parse_relation_type
export classify_by_rules
export calculate_relation_confidence
