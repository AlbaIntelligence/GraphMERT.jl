...
using Dates
using Statistics
using MetaGraphs  # For MetaGraph type
...
function find_connected_components(graph::BiomedicalKnowledgeGraph)
    # Simple connected components implementation without Graphs.jl
    ...
    # Add edges
    for relation in graph.relations
        head_idx = get(entity_id_map, relation.head, 0)
        tail_idx = get(entity_id_map, relation.tail, 0)
        if head_idx > 0 && tail_idx > 0
            push!(adjacency[head_idx], tail_idx)
            push!(adjacency[tail_idx], head_idx)
        end
    end
