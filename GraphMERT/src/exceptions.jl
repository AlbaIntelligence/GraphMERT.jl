...
Base.showerror(io::IO, e::GraphMERTException) = print(io, "GraphMERTException: $(e.message)")
...
struct ModelLoadException <: Exception
    ...
    function ModelLoadException(model_path::String, cause::Union{Exception, Nothing} = nothing)
        new(model_path, cause)
    end
end

Base.showerror(io::IO, e::ModelLoadException) = print(io, "ModelLoadException: Failed to load model from: $(e.model_path)")
...
struct ModelSaveException <: Exception
    ...
    function ModelSaveException(model_path::String, cause::Union{Exception, Nothing} = nothing)
        new(model_path, cause)
    end
end

Base.showerror(io::IO, e::ModelSaveException) = print(io, "ModelSaveException: Failed to save model to: $(e.model_path)")
...
Base.showerror(io::IO, e::DataLoadException) = print(io, "DataLoadException: Failed to load data from: $(e.data_path)")
...
struct APIException <: Exception
    ...
    function APIException(api_name::String, status_code::Int, cause::Union{Exception, Nothing} = nothing)
        new(api_name, status_code, cause)
    end
end

Base.showerror(io::IO, e::APIException) = print(io, "APIException: API call to '$(e.api_name)' failed with status code: $(e.status_code)")
...
struct TextProcessingException <: Exception
    ...
    function TextProcessingException(text::String, cause::Union{Exception, Nothing} = nothing)
        new(text, cause)
    end
end

Base.showerror(io::IO, e::TextProcessingException) = print(io, "TextProcessingException: Text processing failed for: $(e.text[1:min(100, length(e.text))])...")
