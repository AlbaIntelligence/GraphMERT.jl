...
function validate_biomedical_entity(entity_text::String, entity_type::BiomedicalEntityType)
  ...
  elseif entity_type == PERSON
    return occursin(
      r"\b(person|individual|scientist|artist|writer|inventor|leader|figure)\b",
      entity_text_lower,
    ) || occursin(r"\b[A-Z][a-z]+ [A-Z][a-z]+\b", entity_text)  # Name pattern
  elseif entity_type == ORGANIZATION
    return occursin(
      r"\b(organization|company|corporation|institution|university|college|institute|school|government|agency)\b",
      entity_text_lower,
    )
  elseif entity_type == LOCATION
    return occursin(
      r"\b(location|place|city|country|state|region|continent|island|mountain|river|ocean)\b",
      entity_text_lower,
    )
  elseif entity_type == CONCEPT
    return occursin(
      r"\b(concept|idea|principle|theory|method|approach|technique|algorithm|paradigm)\b",
      entity_text_lower,
    )
  elseif entity_type == EVENT
    return occursin(
      r"\b(event|war|revolution|movement|period|era|age|century|decade|conference|meeting)\b",
      entity_text_lower,
    )
  elseif entity_type == TECHNOLOGY
    return occursin(
      r"\b(technology|device|machine|computer|software|hardware|system|platform|tool|instrument)\b",
      entity_text_lower,
    )
  elseif entity_type == ARTWORK
    return occursin(
      r"\b(artwork|painting|sculpture|music|literature|poem|novel|play|film|art|masterpiece)\b",
      entity_text_lower,
    )
  elseif entity_type == PERIOD
    return occursin(
      r"\b(period|era|age|century|decade|epoch|time|phase|stage|generation)\b",
      entity_text_lower,
    )
  elseif entity_type == THEORY
    return occursin(
      r"\b(theory|hypothesis|model|framework|principle|law|theorem|concept|idea)\b",
      entity_text_lower,
    )
  elseif entity_type == METHOD
    return occursin(
      r"\b(method|technique|approach|procedure|process|algorithm|strategy|way|means)\b",
      entity_text_lower,
    )
  elseif entity_type == INSTITUTION
    return occursin(
      r"\b(institution|university|college|institute|school|academy|museum|library|hospital|foundation)\b",
      entity_text_lower,
    )
  elseif entity_type == COUNTRY
    return occursin(
      r"\b(country|nation|state|republic|kingdom|empire|federation|union|territory)\b",
      entity_text_lower,
    )
  else
...
function calculate_entity_confidence(entity_text::String, entity_type::BiomedicalEntityType)
  ...
  elseif entity_type in [
    ...
    if occursin(
      r"\b(historical|cultural|academic|scientific|technological)\b",
      entity_text_lower,
    )
      confidence += 0.1
    end
