...
"""
function perform_local_search(kg::KnowledgeGraph, query::String;
  max_hops::Int=3, max_entities::Int=10)
  ...
  relevant_triples = Vector{Tuple{BiomedicalEntity,BiomedicalRelation,BiomedicalEntity,Float64}}()
    ...
    for (rel_idx, relation) in enumerate(kg.relations)
      for (tail_idx, tail_entity) in enumerate(kg.entities)
        if head_idx != tail_idx
          ...
          total_score = (head_score * 0.4 + tail_score * 0.4 + relation_score * 0.2)

          if total_score > 0.1  # Minimum relevance threshold
            push!(relevant_triples, (head_entity, relation, tail_entity, total_score))
          end
        end
      end
    end
...
"""
function calculate_relation_relevance(relation::BiomedicalRelation, query_terms::Vector{String})
  relation_text = lowercase(relation.relation_type * " " * relation.head * " " * relation.tail)
  score = 0.0
...
"""
function generate_answer_from_context(relevant_triples::Vector{Tuple{BiomedicalEntity,BiomedicalRelation,BiomedicalEntity,Float64}},
  query::String; llm_client::Union{HelperLLMClient,Nothing}=nothing)
  ...
  for (head_entity, relation, tail_entity, score) in relevant_triples
    push!(context_parts, "$(head_entity.text) $(relation.relation_type) $(tail_entity.text)")
  end
...
"""
function evaluate_graphrag(kg::KnowledgeGraph, query::String;
  llm_client::Union{HelperLLMClient,Nothing}=nothing,
  max_hops::Int=3, max_entities::Int=10)

  # Perform local search to find relevant information
  relevant_triples = perform_local_search(kg, query, max_hops=max_hops, max_entities=max_entities)
...
"""
function evaluate_graphrag_dataset(kg::KnowledgeGraph, queries::Vector{String};
  llm_client::Union{HelperLLMClient,Nothing}=nothing)
  scores = Float64[]
