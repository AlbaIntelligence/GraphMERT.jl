...
function evaluate_triple_support(
    ...
    if llm_client !== nothing
        return evaluate_triple_with_llm(
            head_entity,
            relation,
            tail_entity,
            context,
...
function evaluate_triple_heuristic(
    ...
    head_in_context = any(
        occursin(lowercase(head_entity.text), lowercase(sentence)) for sentence in context
    )
    tail_in_context = any(
        occursin(lowercase(tail_entity.text), lowercase(sentence)) for sentence in context
    )

    # Check if relation keywords appear in context
    relation_keywords = get_relation_keywords(relation.relation_type)
    relation_in_context = any(
        occursin(lowercase(keyword), lowercase(join(context, " "))) for
        keyword in relation_keywords
    )
...
function get_relation_keywords(relation_type::String)
    relation_keywords = Dict(
        ...
        "ASSOCIATED_WITH" =>
            ["associated", "related", "linked", "connected", "correlated"],
        "INDICATES" => ["indicate", "suggest", "show", "demonstrate", "reveal"],
        "PREVENTS" => ["prevent", "protect", "block", "inhibit", "reduce"],
        "COMPLICATES" =>
            ["complicate", "worsen", "exacerbate", "aggravate", "complicate"],
    )
...
"""
function calculate_factscore_confidence_interval(
    factscore::Float64,
    n::Int,
