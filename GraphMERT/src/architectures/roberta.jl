...
struct RoBERTaConfig
  ...
  function RoBERTaConfig(;
    ...
    new(vocab_size, hidden_size, num_attention_heads, num_hidden_layers,
...
struct RoBERTaEmbeddings
  ...
  function RoBERTaEmbeddings(config::RoBERTaConfig)
    ...
    new(word_embeddings, position_embeddings, token_type_embeddings, layer_norm, dropout)
  end
end
...
struct RoBERTaSelfAttention
  ...
  function RoBERTaSelfAttention(config::RoBERTaConfig)
    ...
    new(query, key, value, dropout, config.num_attention_heads,
...
struct RoBERTaEncoder
  ...
  function RoBERTaEncoder(config::RoBERTaConfig)
    layers = [RoBERTaLayer(config) for _ in 1:config.num_hidden_layers]
    new(layers, config)
  end
end
...
"""
function (embeddings::RoBERTaEmbeddings)(input_ids::Matrix{Int}, position_ids::Matrix{Int}, token_type_ids::Matrix{Int})
  # Word embeddings
...
"""
function (attention::RoBERTaSelfAttention)(hidden_states::Matrix{Float32}, attention_mask::Matrix{Float32})
...
"""
function (layer::RoBERTaLayer)(hidden_states::Matrix{Float32}, attention_mask::Matrix{Float32})
  # Self-attention
...
"""
function (encoder::RoBERTaEncoder)(hidden_states::Matrix{Float32}, attention_mask::Matrix{Float32})
...
"""
function (model::RoBERTaModel)(input_ids::Matrix{Int}, attention_mask::Matrix{Float32}, position_ids::Matrix{Int}, token_type_ids::Matrix{Int})
  # Embeddings
