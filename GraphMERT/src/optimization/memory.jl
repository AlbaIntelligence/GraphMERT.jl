...
function optimize_memory_usage(monitor::MemoryMonitor)
    ...
    if length(monitor.memory_history) < 2
        return MemoryOptimizationResult(
            monitor.initial_memory,
            monitor.peak_memory,
            monitor.current_memory,
            1.0,
            0.0,
    ...
    suggestions =
        generate_optimization_suggestions(monitor, memory_efficiency, gc_effectiveness)
    ...
    metadata = Dict{String,Any}(
        ...
        "memory_variance" =>
            length(monitor.memory_history) > 1 ? std(monitor.memory_history) : 0.0,
    )
...
function calculate_gc_effectiveness(monitor::MemoryMonitor)
    ...
    for i = 2:length(monitor.memory_history)
        if monitor.memory_history[i] < monitor.memory_history[i-1]
            gc_events += 1
            total_reduction +=
                (monitor.memory_history[i-1] - monitor.memory_history[i]) /
                monitor.memory_history[i-1]
        end
    end
...
function generate_optimization_suggestions(
    ...
    if memory_efficiency < 0.3
        push!(
            suggestions,
            "Memory usage is very high - consider processing smaller batches",
        )
    end
    ...
    if gc_effectiveness < 0.2
        push!(
            suggestions,
            "Garbage collection is not very effective - consider manual memory management",
        )
    end
    ...
    if monitor.gc_triggers > 10
        push!(
            suggestions,
            "Frequent garbage collection detected - optimize data structures",
        )
    end
    ...
    if length(monitor.memory_history) > 5
        ...
        if memory_variance > monitor.max_memory_mb * 0.1
            push!(
                suggestions,
                "High memory variance detected - consider more consistent batch sizes",
            )
        end
    end
...
"""
function optimize_batch_size_for_memory(
    documents::Vector{String},
    initial_batch_size::Int,
    monitor::MemoryMonitor,
)
...
export MemoryMonitor,
    ...
    estimate_memory_per_document_length,
    optimize_memory_usage,
    calculate_gc_effectiveness,
    detect_memory_leaks,
