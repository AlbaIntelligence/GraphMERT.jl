...
function create_empty_chain_graph(config=default_chain_graph_config())
  ...
  root_nodes = [
    ChainGraphNode(:root, config.pad_token_id, i, nothing, true)
  ...
  leaf_nodes = [
    [
      ChainGraphNode(:leaf, config.pad_token_id, config.num_roots + (i - 1) * (config.num_leaves_per_root) + j, i, true)
...
function inject_triple!(graph::LeafyChainGraph, triple::SemanticTriple, root_idx::Int)
  ...
  for (i, token_id) in enumerate(triple.tail_tokens)
    ...
    leaf_pos = graph.config.num_roots + (root_idx - 1) * graph.config.num_leaves_per_root + slot_idx

    # Update leaf node
    graph.leaf_nodes[root_idx][slot_idx] = ChainGraphNode(:leaf, token_id, leaf_pos, root_idx, false)
...
"""
function create_leafy_chain_from_text(text::String, config::ChainGraphConfig=default_chain_graph_config())
  ...
  for i in 1:min(length(tokens), config.num_roots)
