...
"""
function create_performance_monitor(total_steps::Int, max_patience::Int=10)::PerformanceMonitor
...
"""
function update_training_metrics(monitor::PerformanceMonitor, epoch::Int, step::Int,
  total_loss::Float64, mnm_loss::Float64, mlm_loss::Float64,
  learning_rate::Float64, step_time::Float64, tokens_processed::Int)
  ...
  metrics = TrainingMetrics(
    epoch, step, total_loss, mnm_loss, mlm_loss,
    learning_rate, memory_usage, step_time, tokens_processed, throughput, now()
  )
...
function log_detailed_progress(monitor::PerformanceMonitor)
  ...
  @info "Training Progress" epoch = latest.epoch step = latest.step total_steps = monitor.total_steps progress = "$progress_pct%" elapsed_time = "$elapsed_sec s" total_loss = total_loss mnm_loss = mnm_loss mlm_loss = mlm_loss learning_rate = latest.learning_rate memory_usage = "$(memory_mb) MB" step_time = "$(step_time) s" tokens_processed = latest.tokens_processed throughput = "$(throughput) tokens/s" best_loss = best_loss patience = patience_str
end
...
function log_training_summary(monitor::PerformanceMonitor)
  ...
  loss_improvement = round((loss_curve[1] - loss_curve[end]) / max(loss_curve[1], eps()) * 100, digits=3)

  @info "Training Summary" total_epochs = monitor.current_epoch total_steps = monitor.current_step total_tokens = monitor.total_tokens total_time = "$(total_elapsed_sec) s" final_loss = final_loss best_loss = best_loss average_memory = "$(avg_memory) MB" peak_memory = "$(peak_memory) MB" average_throughput = "$(avg_throughput) tokens/s" peak_throughput = "$(peak_throughput) tokens/s" loss_improvement = "$(loss_improvement)%"
end
...
function save_training_log(monitor::PerformanceMonitor, log_path::String)
  ...
  open(log_path, "w") do io
    ...
    write(io, "Epoch,Step,Total_Loss,MNM_Loss,MLM_Loss,Learning_Rate,Memory_MB,Step_Time_s,Throughput_tokens_s\n")

    for metrics in monitor.metrics_history
      write(io, "$(metrics.epoch),$(metrics.step),$(metrics.total_loss),$(metrics.mnm_loss),$(metrics.mlm_loss),$(metrics.learning_rate),$(metrics.memory_usage),$(metrics.step_time),$(metrics.throughput)\n")
    end
  end
...
export TrainingMetrics, PerformanceMonitor, create_performance_monitor,
  update_training_metrics, log_detailed_progress, should_early_stop,
  get_loss_curve, get_memory_usage, get_throughput, log_training_summary,
  save_training_log, log_training_progress, log_evaluation_metrics,
